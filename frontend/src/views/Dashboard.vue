...
// STATO PULL-TO-REFRESH
const startY = ref(0);
const pullDistance = ref(0);
const isRefreshing = ref(false);
const PULL_THRESHOLD = 70; // Soglia per attivare l'aggiornamento
...
// --- GESTIONE TOUCH PER PULL-TO-REFRESH ---
...
const handleTouchMove = (e) => {
  ...
  if (diff > 0) {
    // Resistenza elastica: tetto massimo a 100px di scorrimento
    pullDistance.value = Math.min(diff * 0.45, 100);
  }
};
...
const handleTouchEnd = async () => {
  if (pullDistance.value >= PULL_THRESHOLD && !isRefreshing.value) {
    ...
    pullDistance.value = PULL_THRESHOLD; // Blocca la pagina in basso durante il caricamento
    ...
    setTimeout(() => {
      ...
      startY.value = 0;
    }, 500); // Ritardo extra per apprezzare l'animazione di completamento
  } else {
    pullDistance.value = 0;
    startY.value = 0;
  }
...
<template>
  <div class="min-h-screen bg-slate-50 dark:bg-slate-900 overflow-x-hidden font-sans text-slate-800 dark:text-slate-100 selection:bg-brand selection:text-white pb-20 transition-colors duration-300">
    ...
    <!-- HEADER (Fisso, fuori dal wrapper che si muove) -->
    ...
    <!-- WRAPPER DEL CONTENUTO CHE SCORRE IN BASSO -->
    <div 
    ...
    >
      ...
      <div class="absolute left-0 right-0 -top-16 flex items-center justify-center h-16 w-full pointer-events-none z-0">
        <div 
          ...
          :class="pullDistance > 10 ? 'opacity-100' : 'opacity-0'"
        >
          <svg 
            v-if="isRefreshing" 
          ...
          <svg 
            v-else 
            class="w-5 h-5 text-brand transition-transform duration-200"
